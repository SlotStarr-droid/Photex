...
export async function analyzeImage(uri: string, mimeType = "image/jpeg"): Promise<AIAnalysis> {
  let base64: string;
  ...
  const data = (await response.json()) as { analysis: AIAnalysis; model: string; analyzedAt: string };
  return { ...data.analysis, model: data.model, analyzedAt: data.analyzedAt };
}
